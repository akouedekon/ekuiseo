  ...
  return (
    <div className={cn('rounded-[var(--radius-card)] border border-rule bg-surface', className)}>
      <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1 px-4 pt-4">
        <span className="text-body text-muted">Prix total du voyage</span>
        <span className="tnum font-display text-display font-extrabold tracking-[-0.03em]">
      ...
      <div className={cn('mt-3 grid gap-px overflow-hidden bg-rule', !cashOnly && !fullyOnline && 'sm:grid-cols-2')}>
        ...
        {!cashOnly ? (
          <div className="bg-primary-soft p-4">
            <span className="flex items-center gap-1.5 text-caption font-bold uppercase tracking-[0.06em] text-primary-ink">
              <Smartphone className="size-3.5" aria-hidden />
              {fullyOnline ? 'En ligne — maintenant' : 'Étape 1 — maintenant'}
            </span>
            <p className="tnum mt-1.5 font-display text-display font-extrabold leading-none tracking-[-0.03em] text-primary-ink">
              {formatFcfa(plan.depositAmount)}
            </p>
            <p className="mt-1.5 text-label leading-snug text-primary-ink">
              {fullyOnline
        ...
        {!fullyOnline ? (
          <div className="bg-surface-2 p-4">
            <span className="flex items-center gap-1.5 text-caption font-bold uppercase tracking-[0.06em] text-ink-2">
              <Banknote className="size-3.5" aria-hidden />
              {cashOnly ? 'En espèces — à bord' : 'Étape 2 — à bord'}
            </span>
            <p className="tnum mt-1.5 font-display text-display font-extrabold leading-none tracking-[-0.03em]">
              {formatFcfa(plan.balanceAmount)}
            </p>
            <p className="mt-1.5 text-label leading-snug text-ink-2">
              {cashOnly
      ...
      {!compact ? (
        <div className="px-4 py-3">
          ...
          <p className="flex items-start gap-2 text-label leading-relaxed text-muted">
