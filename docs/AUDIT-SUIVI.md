...
- **SMS** (rattaché à F107, F234, F605) — Aucun fournisseur SMS n est choisi : les codes de connexion partent par e-mail, le SMS ne sert qu aux notifications critiques et au repli OTP si `SMS_MODE=http`. Choisir le fournisseur (SMS Partner, Africa s Talking ou Twilio, gateways déjà écrites), renseigner les variables et décider si `OTP_SMS_FALLBACK` passe à `true` ; c est aussi ce qui permettra de vérifier un compte mobile money sur un numéro différent du numéro de connexion.
